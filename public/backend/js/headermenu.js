...
$(document).on('click', '.editBtn', function (event) {
    ...
    $.post(url, data, function (data) {
        if (data) {
            blankData();
            toastr.success($('#operation_success').val(), {timeOut: 5000,});
            reloadWithData(data);
        } else {
            toastr.error( $('#operation_faild').val(), {timeOut: 5000,});
        }
    });
...
$(document).ready(function () {
    $(document).on('click', '#add_custom_link_btn', function (event) {
        ...
        $.post(url, data, function (data) {
            if (data) {
                blankData();
                toastr.success($('#operation_success').val());
                reloadWithData(data);
            } else {
                toastr.error($('#operation_faild').val());
            }
        });
    });
});
    ...
    $(document).on('mouseover', 'body', function () {
        ...
        $('.dd').nestable({
            ...
            callback: function (l, e) {
                ...
                $.post(url, data, function (data) {
                    if (data != 1) {
                        toastr.error($('#error_mesg').val(), {timeOut: 5000,});
                    }
                });
            }
        });
...
function checkDemo(){
    ...
    if (demoMode) {
        toastr.warning("For the demo version, you cannot change this");
        return false;
    }else {
        return true;
    }
